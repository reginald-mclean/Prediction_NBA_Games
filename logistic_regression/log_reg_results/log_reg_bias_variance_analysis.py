...
def bias_variance_analysis(file_name, C, reg, solver, l1_ratio, num_trials=10):
    accuracy_train = []
    precision_train = []
    recall_train = []
    f1_train = []
    ...
    for trial in range(num_trials):
        ...
        model = LogisticRegression(penalty=reg, C=C, max_iter=10000, l1_ratio=l1_ratio, solver=solver)
...
def process_data(file_path, reg, solver, write_file):
    ...
    if reg == 'elasticnet':
        l1_ratio_params = parameter_data['l1_ratio'].to_numpy()
    else:
        l1_ratio_params = [None] * len(file_names)
    for index, read_file in enumerate(file_names):
        print('regularizer:', reg)
        stats_df = bias_variance_analysis(read_file, reg_params[index], reg, solver, l1_ratio_params[index])
...
if __name__ == '__main__':
    # files = ['log_reg_elasticnet_best_hyperparameters.csv',
    #          'log_reg_l1_best_hyperparameters.csv',
    #          'log_reg_l2_best_hyperparameters.csv']
    # write_files = ['log_reg_elasticnet_bias_variance.csv',
    #                'log_reg_l1_bias_variance.csv',
    #                'log_reg_l2_bias_variance.csv']
    # regularizers = ['elasticnet', 'l1', 'l2']
    # solvers = ['saga', 'liblinear', 'lbfgs']
    #
    # for i, file in enumerate(files):
    #     process_data(file, regularizers[i], solvers[i], write_files[i])

    """
    no regularizer
    """
    file_names = ['./Data/normalized_5_game_average_final.csv',
    ...
    for index, read_file in enumerate(file_names):
        ...
        stats_df = bias_variance_analysis(read_file, 1.0, 'none', 'lbfgs', None)
        if statistic_summary.empty:
            statistic_summary = pd.DataFrame(columns=stats_df.keys())
        statistic_summary = pd.concat([statistic_summary, stats_df], ignore_index=True)
    summary['file'] = file_names
    summary = pd.concat([summary, statistic_summary], axis=1)
    summary.to_csv('log_reg_no_reg_bias_variance.csv')